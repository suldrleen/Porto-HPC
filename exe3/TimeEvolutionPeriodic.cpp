...
Eigen::MatrixXd H(double t_value, unsigned N) {
    ...
    for(int i = 0; i < N; i++) 
    {
        H(i, (i + 1) % N) = -t_value; // élément supérieur, condition périodique
        H(i, (i - 1 + N) % N) = -t_value; // élément inférieur
    }
    return H;
}
...
int main() 
{
    ...
    Eigen::MatrixXd H_matrix = H(t, N); // Calculer la matrice H
