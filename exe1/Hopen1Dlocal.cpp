    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < N; i++) 
        {
            ...
            for (int y = 1; y <= N; y++)
            {
                ...
                dos_private(y - 1) += (gamma / ((energy - en(i)) * (energy - en(i)) + gamma * gamma)) * phi * phi;
            }
        }
...
int main()
{
    ...
    for (int idx = 0; idx < thread_counts.size(); ++idx)
    {
        ...
        std::ofstream DosFile("DosOpen1Dlocal_" + std::to_string(threads) + ".txt");
        if (!DosFile) {
    ...
    std::ofstream SpeedupFile("speedup_eigen.txt");
